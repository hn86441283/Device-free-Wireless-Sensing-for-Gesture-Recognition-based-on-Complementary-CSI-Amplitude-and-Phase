...
for i = 1:database_length%length of csi data
    ...
    csi_phase = angle(csi_j);          %Extract magnitude (dimensionality reduction + transposition)
    %% Extract phase or amplitude
    first_ant_csi(:,i) = phase_calibration(csi_phase(:,1)');           %Fetch the first column of data directly (no need to for loop to fetch)
    second_ant_csi(:,i) = phase_calibration(csi_phase(:,2)');
    third_ant_csi(:,i) = phase_calibration(csi_phase(:,3)');
end
%% Anomaly removal 3¦Ò
for kk = 1:30
...
Sample_set(100*numrot+num_file,1:end-1) = Samples;
Sample_set(100*numrot+num_file,end) = gesture;
end
end